...
from web.server import app as flask_app  # Renommé en flask_app pour éviter le conflit
...
def start_web_server():
    ...
    flask_app.run(host='0.0.0.0', port=5000, debug=False)  # Utilise flask_app au lieu de app
...
def main():
    ...
    try:
        qt_app = QApplication(sys.argv)  # Renommé en qt_app pour éviter le conflit

        # Définir une police par défaut
        QFontDatabase.addApplicationFont("C:/Windows/Fonts/Arial.ttf")
        qt_app.setFont(QFont("Arial", 12))
        ...
        if stay_logged_in and saved_username and saved_password:
            if auth_manager.authenticate(saved_username, saved_password):
                ...
                sys.exit(qt_app.exec())  # Utilise qt_app au lieu de app
        ...
        # Arrêter serveur Flask à la fermeture de l'app
